        ...
        while True:
            ...
            for c_node in children:
                ...
                if (not key  in self.closed) and (not key in self.frontier_log): 
                    self.frontier.append(c_node)
                    self.frontier_log[key] = True
                # testa se o custo de c_node que ta na borda e menor e subsitui o elemento
                else:
                    for n in self.frontier:
                        if n.state == c_node.state and c_node.path_cost < n.path_cost:
                            self.frontier[n.index()] = c_node
            # ordena a borda pelo menor custo                 
            self.frontier = sorted(self.frontier, key = lambda node: node.path_cost)
...
if __name__ == "__main__":
    solver = simple_problem_solver(simple_graph_jar_agent_busca_uniforme())
    env = Environment()
